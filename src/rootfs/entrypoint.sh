...
default_uid='1000'
default_gid='1000'
default_unprivileged_user='github'
...
# Rename the runas user if it doesn't match the default
...
# Change the runas user and group to the specified UID and GID if they are not the default
if { [ ! -z "${PUID}" ] && [ "${PUID}" != "$default_uid" ]; } || { [ ! -z "${PGID}" ] && [ "${PGID}" != "$default_gid" ]; }; then
    ...
    chown "${PUID}:${PGID}" "/home/${run_as_user}" "/github" "/config"
